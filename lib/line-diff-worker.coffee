...
class LineDiffWorker
    ...
    decorateDiffMarkers: (details) ->
        ...
        marker = null
        messageBubble = null
        marker = @editor.markBufferRange([startPoint, endPoint], {name: "line-diff"})
    ...
    findFileDiffs: ->
        ...
        activeEditorText = atom.workspace.getActiveEditor().getBuffer().getText()
    ...
    findDiffForLine: (lineNumber) ->
        ...
        for diff in fileDiffs
            if (diff.newLineNumber is lineNumber) or (diff.newLineNumber is -1 and diff.oldLineNumber is lineNumber + 1)
                return diff
        return {}
